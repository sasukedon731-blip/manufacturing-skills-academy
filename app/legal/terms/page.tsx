    ...
    <main style={styles.main}>
      <article style={styles.card}>
        ...
        <Section title="第1条（適用）">
          本規約は、株式会社アウトインプラス（以下「当社」といいます）が提供するManufacturing Skills Academy（以下「本サービス」といいます）の利用条件を定めるものです。ユーザーは本規約に同意のうえ、本サービスを利用するものとします。
        </Section>

        <Section title="第2条（サービス内容）">
          本サービスは、製造現場で使う日本語、製造用語、JLPT学習、ゲーム形式の学習、AI会話・AIスピーキング等の学習機能を提供します。AI会話・AIスピーキングは、別料金のAIオプションとして提供される場合があります。
        </Section>

        <Section title="第3条（アカウント管理）">
          ユーザーは、自己の責任においてアカウント情報を管理するものとします。第三者による不正利用が疑われる場合は、速やかに当社へ連絡してください。
        </Section>

        <Section title="第4条（料金および支払い）">
          有料機能は、買い切り型の期間利用権として販売されます。購入時に選択した期間と内容に応じて、クレジットカードまたはコンビニ決済で支払いが発生します。
        </Section>
        ...
        <Section title="第6条（返金）">
          デジタルコンテンツの性質上、決済完了後のお客様都合による返金は原則としてお受けしておりません。ただし、二重課金や重大な不具合等、当社が必要と判断した場合は個別に対応します。
        </Section>

        <Section title="第7条（禁止事項）">
          法令または公序良俗に反する行為、不正アクセス、第三者になりすます行為、教材・画像・音声・文章等の無断転載、複製、配布、販売、その他当社が不適切と判断する行為を禁止します。
        </Section>

        <Section title="第8条（免責）">
          本サービスは学習支援を目的とするものであり、特定の試験合格、資格取得、就職等を保証するものではありません。AI機能の出力内容は、正確性・完全性を保証するものではありません。
        </Section>

        <Section title="第9条（サービスの変更・停止）">
          当社は、保守、障害対応、仕様変更、その他必要がある場合、本サービスの全部または一部を変更または停止することがあります。
        </Section>

        <Section title="第10条（規約の変更）">
          当社は必要に応じて本規約を変更できます。変更後の内容は、本サービス上に表示した時点で効力を生じます。
        </Section>
